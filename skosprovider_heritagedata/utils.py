...
for csd in data:
    cs = ConceptScheme(
        csd['uri'],
        labels=[Label(csd['label'], 'prefLabel', csd['label lang'])],
        notes=[Note(csd['description'], 'scopeNote', csd['description lang'])],
        sources=[Source(csd['attribution'], None)],
    )
...
def _create_note(literal, type):
    ...
    return Note(
        text_(literal.value, encoding='utf-8'), type, _get_language_from_literal(literal)
    )
