...
class HeritagedataProviderTests(unittest.TestCase):
    ...
    def test_get_custom_vocabulary_uri(self):
        ...
        assert (
            'http://purl.org/heritagedata/schemes/scapa' == provider.get_vocabulary_uri()
        )
    ...
    def test_default_provider(self):
        ...
        self.assertEqual(
            provider.service_scheme_uri, 'http://heritagedata.org/live/services'
        )

    def test_scheme_uri_not_available(self):
        provider = HeritagedataProvider(
            {'id': 'Heritagedata'},
            service_scheme_uri='http://heritagedata.org/live/services_not_available/',
        )
        self.assertEqual(provider.base_scheme_uri, 'http://purl.org/heritagedata/schemes')
        self.assertEqual(provider.scheme_id, 'eh_period')
        self.assertEqual(
    ...
    def test_default_language_scottish_gaelic(self):
        ...
        provider_en = HeritagedataProvider(
            {'id': 'Heritagedata'}, scheme_uri='http://purl.org/heritagedata/schemes/1'
        )
    ...
    def test_get_by_id_concept(self):
        ...
        concept = concept.__dict__
        self.assertEqual(
            concept['uri'], 'http://purl.org/heritagedata/schemes/eh_period/concepts/PM'
        )
    ...
    def test_get_by_uri(self):
        concept = HeritagedataProvider(
            {'id': 'Heritagedata'},
            scheme_uri='http://purl.org/heritagedata/schemes/eh_period',
        ).get_by_uri('http://purl.org/heritagedata/schemes/eh_period/concepts/PM')
        concept = concept.__dict__
        self.assertEqual(
            concept['uri'], 'http://purl.org/heritagedata/schemes/eh_period/concepts/PM'
        )
    ...
    def test_get_children_display_sort(self):
        ...
        sorted_by_label_desc = prov.get_children_display(
            'PM', sort='label', sort_order='desc'
        )
    ...
    def test_find_multiple_keywords(self):
        r = HeritagedataProvider(
            {'id': 'Heritagedata'},
            scheme_uri='http://purl.org/heritagedata/schemes/eh_period',
        ).find({'label': 'iron Age', 'type': 'concept'})
    ...
    def test_find_one_keyword(self):
        r = HeritagedataProvider(
            {'id': 'Heritagedata'},
            scheme_uri='http://purl.org/heritagedata/schemes/eh_period',
        ).find({'label': 'VICTORIAN', 'type': 'concept'})
        self.assertIsInstance(r, list)
        self.assertGreater(len(r), 0)
        for res in r:
            self.assertEqual(res['type'], 'concept')
    ...
    def test_find_one_keyword_language(self):
        ...
        r = HeritagedataProvider(
            {'id': 'Heritagedata'},
            scheme_uri='http://purl.org/heritagedata/schemes/eh_period',
        ).find({'label': 'VICTORIAN', 'type': 'concept'}, **kwargs)
        self.assertIsInstance(r, list)
        self.assertGreater(len(r), 0)
        for res in r:
            self.assertEqual(res['type'], 'concept')
    ...
    def test_find_kwargs_language(self):
        kwargs = {'language': 'gd'}
        provider_gd = HeritagedataProvider(
            {'id': 'Heritagedata', 'default_language': 'en'},
            scheme_uri='http://purl.org/heritagedata/schemes/1',
        )
    ...
    def test_get_items(self):
        ...
        self.assertEqual(
            res[0]['uri'], 'http://purl.org/heritagedata/schemes/eh_period/concepts/VIC'
        )

    def test_get_items_non_exist_service(self):
        provider = HeritagedataProvider(
            {'id': 'Heritagedata'},
            service_scheme_uri='http://heritagedata.org/live/services2/',
        )
        self.assertRaises(
            ProviderUnavailableException,
            provider._get_items,
    ...
    def test_get_items_non_exist_service2(self):
        ...
        self.assertRaises(
            ProviderUnavailableException, provider._get_items, 'invalid', {}
        )
    ...
    def test_get_items_wrong_params(self):
        ...
        self.assertRaises(
            ValueError, provider._get_items, 'getConceptLabelMatch', {'fhgfhg'}
        )
